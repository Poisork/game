...
class App extends Component {
  ...
  setNewIntervalAndResetOld = () => {
    const field = this.gameModeRef.current.value
      ? JSON.parse(this.gameModeRef.current.value).field
      : ''
  ...
  whenGameEnd = nameWinner => {
    clearInterval(this.timeout)
    this.setState({endGame: {nameWinner, endGameValue: true}})
    const data = {
  ...
  userCatchColum = ({target: {dataset}}) => {
    ...
    if (randomNumber === id && !endGameValue && !arrayUser.includes(id)) {
      this.setState(prevState => {
        return {arrayUser: [...prevState.arrayUser, id]}
      })
    }
  }

  scoreComparison() {
    const {arrayUser, powerField, name, randomNumber} = this.state
    if (randomNumber) {
      const arr = [...Array(powerField).keys()].map(el => el + 1)
      const arrayComputer = arr.filter(
        el => !uniqueRandoms.includes(el) && !arrayUser.includes(el),
      )
      const percentageExpressionUser = (arrayUser.length * 100) / powerField
      const percentageExpressionComputer =
        (arrayComputer.length * 100) / powerField
  ...
  render() {
    const {
      ...
      winners,
      randomNumber,
      gameModeRefObj: {field},
      endGame: {nameWinner, endGameValue},
    } = this.state
    return (
      <div className='App'>
        <section className='side'>
          <header className='side__header'>
            {validationError ? (
              <p>Enter a name and select the difficulty of the game</p>
            ) : (
              ''
            )}
            {Object.entries(settings) ? (
              <select className='side__select' ref={this.gameModeRef}>
                <option className='side__option' value=''>
                  Pick game mode
                </option>
            ...
            <input
              ...
              type='submit'
              value={endGameValue ? 'PLAY AGAIN' : 'PLAY'}
              onClick={this.setNewIntervalAndResetOld}
            />
          </header>
          {endGameValue ? <h2>{`${nameWinner} Win!`}</h2> : null}
          {field ? (
            <Context.Provider
              value={{arrayUser, randomNumber, endGameValue, arrayComputer}}
            >
              <Table field={field} userCatchColum={this.userCatchColum} />
            </Context.Provider>
          ) : null}
        </section>
